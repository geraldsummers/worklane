...
if ! pgrep -u "$(id -u)" -f "$watcher" >/dev/null 2>&1; then
  split_output="$(herdr --session "$WORKLANE_NAME" pane split --direction right --ratio 0.5 --cwd "$HOME/$WORKLANE_NAME" --env "WORKLANE_NAME=$WORKLANE_NAME" --no-focus 2>/dev/null || true)"
...
mod tests {
    ...
    fn herdr_bootstrap_uses_the_lane_name_as_its_workspace_label() {
        ...
        assert!(source.contains("pane split --direction right --ratio 0.5"));
